"""
FIX: Restores all missing webhook tools + switches to faster LLM (gemini-2.0-flash)
Root cause: webhook tools were wiped from the agent config, leaving only transfer_to_agent
Result: Sofia was transferring every call to Marco because it was the only tool available
"""
import json
import requests
...
The transfer_to_agent tool MUST NEVER be called unless ALL THREE conditions below are simultaneously true:
  ...
  3. The issue is truly beyond your authority (emergency refunds, complex legal disputes)
...
WEBHOOK_TOOLS = [
    {
        ...
        "description": (
            ...
            "Do NOT ask 'are you looking for departure or return?' or 'which city?' before calling. "
    ...
    },
    {
        ...
        "description": (
            ...
            "IMPORTANT: Call immediately with whatever the passenger gave. "
...
def patch():
    print(f"\n--- Fixing: restoring {len(WEBHOOK_TOOLS)} webhook tools + switching to gemini-2.0-flash ---")
        ...
        "conversation_config": {
            "agent": {
                "prompt": {
                    ...
                    "llm": "gemini-2.0-flash",        # faster than gemini-3-flash-preview
                    "temperature": 0.5,
                    "max_tokens": -1,
    ...
    if resp.status_code in [200, 201]:
        ...
        print(f"  [OK] Patched!")
        print(f"  Tools saved: {len(saved_tools)} (expected {len(ALL_TOOLS)})")
        print(f"  LLM: {llm}")
        webhook_count = sum(1 for t in saved_tools if t.get("type") == "webhook")
        print(f"  Webhook tools: {webhook_count}")
        print(f"  System tools: {len(saved_tools) - webhook_count}")
    else:
...
if __name__ == "__main__":
    patch()
    print("\n=== Fix applied. Retry Conversation A now. ===")
