...
Step 1 — Call get_available_seats to retrieve available seats for the booking. You may filter by cabin_class if the passenger specified a preference (window, aisle, etc.).
Step 2 — Describe a few relevant options to the passenger naturally (e.g. "I have 12A, a window seat, or 14C on the aisle").
...
Step 3 — Wait for the passenger to choose (cash or points) and confirm.
...
def update_agent():
    print(f"\n--- Phase 4: Updating agent with {len(ALL_TOOLS)} tools ---")
    ...
    if resp.status_code in [200, 201]:
        ...
        print(f"  [OK] Agent updated! Tools saved: {len(saved_tools)}")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("\nTest with:")
        print("  1. Auth XKRTMN/Rossi → 'I want to change my seat to a window seat'")
        print("     → Sofia calls get_available_seats, describes options, confirms any fee")
        print("  2. Auth XKRTMN/Rossi → 'I want to upgrade to Business class'")
        print("     → Sofia calls upgrade_cabin, presents cash price AND SkyMiles option")
    else:
        print("\nPhase 4 failed.")
